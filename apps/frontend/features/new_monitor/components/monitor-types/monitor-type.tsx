    ...
    <div
      ...
      className={`relative border rounded-lg transition-[border-color,background-color] duration-150 ease-in-out ${
        comingSoon
          ...
          : isActive
            ? "border-[1.5px] border-sf-text bg-[#f8f8ff] cursor-pointer"
            : "border-[1.5px] border-sf-border hover:border-gray-300 hover:bg-sf-bg cursor-pointer"
      }`}
    >
      {comingSoon && (
        <span className="absolute top-2.5 right-2.5 text-[10px] font-semibold font-sans px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-400 tracking-wide">
          Soon
        </span>
      )}
      <div className="flex flex-col p-4">
        <div
          className={`w-fit p-1.5 rounded-[6px] transition-colors duration-150 ease-in-out ${
            isActive && !comingSoon ? "bg-sf-text" : "bg-gray-100"
          }`}
        >
          <Icon
            className={`w-4 h-4 transition-colors ${
              isActive && !comingSoon ? "text-white" : "text-gray-400"
            }`}
          />
        </div>
