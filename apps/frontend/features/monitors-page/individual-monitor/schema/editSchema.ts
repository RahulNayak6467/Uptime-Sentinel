...
export const editSchema = z.object({
  ...
  requestBodyType: z
    .enum(["none", "json", "form-encoded", "raw-text"], {
      error: "Request body type is not supported"
    }),
  ...
  if (data.httpMethod === "GET") {
    if (
      data.requestBodyType !== "none" ||
      data.contentType !== "none" ||
      (data.requestBody !== null && data.requestBody.trim().length > 0)
    ) {
      context.addIssue({
        code: "custom",
        path: ["requestBodyType"],
  ...
  if (data.requestBodyType === "none") {
    if (data.contentType !== "none") {
      context.addIssue({
        code: "custom",
        path: ["contentType"],
        message: "Content-Type must be none when no body is configured",
      });
    }
    if (data.requestBody !== null && data.requestBody.trim().length > 0) {
      context.addIssue({
        code: "custom",
        path: ["requestBody"],
        message: "Remove the request body or select a body type",
      });
    }
    return;
  }

  if (data.contentType !== expectedContentType[data.requestBodyType]) {
    context.addIssue({
      code: "custom",
      path: ["contentType"],
      message: "Content-Type does not match the selected body type",
    });
  }
  ...
  if (data.requestBody === null || data.requestBody.trim().length === 0) {
    context.addIssue({
      code: "custom",
      path: ["requestBody"],
      message: "Enter a request body or select None",
    });
    return;
  }

  if (data.requestBodyType === "json") {
    try {
